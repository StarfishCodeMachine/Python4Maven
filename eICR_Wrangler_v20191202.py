#--@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#--@@@ eICR Wrangler for Maven by Wes McNeely 12/2/2019                                                
#--@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
#--@@@ 					of the Implementation Guide and were tailored to match xPaths found in Epic-derived  
...
#--@@@  
#--@@@				Finally, I recommend executing this script within Sublime Text 3 using "ctrl-B". Sublime     
#--@@@ 					must be configured to recognize Python in order to do this. 
#--@@@  
#--@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#--@@@  Updates:
#--@@@  
...
##### Functions for logging, copying files to new folders, #########
#####   purging folders, and retrieving data from XML elements    ##
...
inputPath = 'K:\\Wes\\python\\projects\\eICR_Wrangler\\EICR\\'      			#<--- raw eICRs
mIFPath = 'k:\\wes\\python\\projects\\eICR_Wrangler\\MIF\\' 					#<--- initial MIF storage (purged every time program is run)
cSVPath = 'k:\\wes\\python\\projects\\eICR_Wrangler\\CSV\\' 					#<--- csv storage for troubleshooting pesky eICRs (purged every time program is run)
summaryPath = 'k:\\wes\\python\\projects\\eICR_Wrangler\\LOGS\\'				#<--- log files
mavenInputPath1 = '\\\\TF12B133APP01\\ecr\\INPUT\\'								#<--- MIFs ready to ingest (on Maven app server)

##### Set Vars  ####################################################

fileCounter=0
fileCounterLimit = 200		#<--- this will limit the number of files processed to 200
...
##### Problems #####################################################
		...
		for problem in problems:
			...
			problemNotedeffectiveDate = xml_retrieve_date_fromstring(problem,'.//effectiveTime/low','value')
			problemObservedeffectiveDate = xml_retrieve_date_fromstring(problem,'.//entryRelationship/observation/effectiveTime/low','value')

			#problemCode = xml_retrieve_from_elem(problem,'.//entryRelationship/observation/value/translation[@codeSystemName="ICD-10-CM" or @codeSystemName="ICD10"]', 'code')
			#problemDescription = xml_retrieve_from_elem(problem,'.//entryRelationship/observation/value/translation[@codeSystemName="ICD-10-CM" or @codeSystemName="ICD10"]','displayName')

			try:
				problemCode = problem.find('.//entryRelationship/observation/value/translation[@codeSystemName="ICD-10-CM"]').get('code')
				problemDescription = problem.find('.//entryRelationship/observation/value/translation[@codeSystemName="ICD-10-CM"]').get('displayName')

			except Exception as e:
				custom_logger('ICD-10-CM' + str(e))
				try:				
					problemCode = problem.find('.//entryRelationship/observation/value/translation[@codeSystemName="ICD10"]').get('code')
					problemDescription = problem.find('.//entryRelationship/observation/value/translation[@codeSystemName="ICD10"]').get('displayName')

				except Exception as e:
					problemCode = '--'
					problemDescription = '--'
					custom_logger('ICD10' + str(e))

			print(         'Problem: ',  problemNotedeffectiveDate, problemObservedeffectiveDate, problemCode, problemDescription )
			csvDataLine = ['Problem: ',  problemNotedeffectiveDate, problemObservedeffectiveDate, problemCode, problemDescription]
			csvWriter.writerow(csvDataLine)

			problemIterationStr =  str(int(problemCounter))

			RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=problemIterationStr, QuestionID='ECR_PROBLEM_NOTED_DATE', Value=problemNotedeffectiveDate)
			RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=problemIterationStr, QuestionID='ECR_PROBLEM_OBSERVED_DATE', Value=problemObservedeffectiveDate)
			RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=problemIterationStr, QuestionID='ECR_PROBLEM_ICD10_CODE', Value=problemCode)
			RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=problemIterationStr, QuestionID='ECR_PROBLEM_ICD10_DESCR', Value=problemDescription)
...
##### Medications ##################################################
		...
		for medication in medications:
			...
			medicationStartDate = xml_retrieve_date_fromstring(medication,'.//effectiveTime/low','value')

			medicationStopDate = xml_retrieve_date_fromstring(medication,'.//effectiveTime/high','value')
			#medicationText = xml_retrieve_from_elem(medication,'.//text')

			medicationName = xml_retrieve_from_elem(medication,'.//consumable/manufacturedProduct/manufacturedMaterial/code/originalText') + xml_retrieve_from_elem(medication,'.//consumable/manufacturedProduct/manufacturedMaterial/code/translation', 'displayName') 

			medicationRoute = xml_retrieve_from_elem(medication,'.//routeCode','displayName')

			medicationdoseQuantity = xml_retrieve_from_elem(medication,'.//doseQuantity','value') + xml_retrieve_from_elem(medication,'.//doseQuantity','unit')
			medicationFrequency = xml_retrieve_from_elem(medication,'.//period','value') + xml_retrieve_from_elem(medication,'.//period','unit')

			medicationCounterStr =  str(int(medicationCounter))

			RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=medicationCounterStr, QuestionID='ECR_MEDICATION_DATESTART', Value=medicationStartDate)
			RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=medicationCounterStr, QuestionID='ECR_MEDICATION_DATESTOP', Value=medicationStopDate)
			...
			RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=medicationCounterStr, QuestionID='ECR_MEDICATION_FREQUENCY', Value=medicationFrequency)
...
##### Immunizations ################################################
		...
		immunizationsSection = tree.find('.//templateId[@root="2.16.840.1.113883.10.20.22.4.52"]/..') #finds the parent to the <title/> tag
		immunizationsCounter = 0 
		print(immunizationsSection)
		custom_logger('immunizationsSection1' + ' ' +str(immunizationsSection))
		try:				

			if not immunizationsSection is None:

				immunizations = immunizationsSection.xpath('.//entry/substanceAdministration')
				print(immunizationsSection)				
				custom_logger('immunizationsSection2' + ' ' +str(immunizations))

				for immunization in immunizations:
					custom_logger('immunizationsSection3' + ' ' +str(immunization))

					immunizationDate = xml_retrieve_date_fromstring(immunization,'.//effectiveTime','value')
					immunizationProductdisplayName = xml_retrieve_from_elem(immunization,'.//consumable/manufacturedProduct/manufacturedMaterial/code/translation','displayName') 
					immunizationRouteCode = xml_retrieve_from_elem(immunization,'.//routeCode','code')
					immunizationRouteDescr = xml_retrieve_from_elem(immunization,'.//routeCode','displayName')
					immunizationDoseQuantity = xml_retrieve_from_elem(immunization,'.//doseQuantity','value') + ' ' + xml_retrieve_from_elem(immunization,'.//doseQuantity','unit')
					immunizationProductCode = xml_retrieve_from_elem(immunization,'.//consumable/manufacturedProduct/manufacturedMaterial/code/translation','code') 
					immunizationProductCodeSystem = xml_retrieve_from_elem(immunization,'.//consumable/manufacturedProduct/manufacturedMaterial/code/translation','codeSystemName') 
					immunizationProductLotNumber = xml_retrieve_from_elem(immunization,'.//consumable/manufacturedProduct/manufacturedMaterial/lotNumberText') 
					immunizationProductmanufacturerOrganization = xml_retrieve_from_elem(immunization,'.//consumable/manufacturedProduct/manufacturerOrganization/name') 
					...
					RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=immunizationsCounterStr, QuestionID='ECR_IMMUNIZATION_NAME', Value=immunizationProductdisplayName )
					...
					RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=immunizationsCounterStr, QuestionID='ECR_IMMUNIZATION_PRODUCTCODE', Value=immunizationProductCode )
					RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=immunizationsCounterStr, QuestionID='ECR_IMMUNIZATION_CODESYSTEM', Value=immunizationProductCodeSystem )
					RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=immunizationsCounterStr, QuestionID='ECR_IMMUNIZATION_LOT', Value= immunizationProductLotNumber)
					RiskDataDefinition = etree.SubElement(ParticipantDefinition, 'RiskDataDefinition', Iteration=immunizationsCounterStr, QuestionID='ECR_IMMUNIZATION_MANUFACTURER', Value=immunizationProductmanufacturerOrganization )
					immunizationsCounter +=1

			else:
				pass
		except Exception as e:
			custom_logger('immunizationsSection' + ' ' +str(e))
			pass		
